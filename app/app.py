...
def blogpost(post_id):
	...
	file_data = BlogPost.query.filter_by(post_id=post_id).first()
	image = b64encode(file_data.image) 
	filez_data = BlogPost.query.filter_by(post_id=post_id).first()
	image2 = b64encode(filez_data.image2) 
	return render_template('detail.html', post=post, image = image.decode('utf8'), image2= image2.decode('utf8'))
...
def addpost():
   if request.method == 'POST':
       ...
       g = request.files['filez']
       post = BlogPost(title=title, image=f.read(), image2=g.read(), content=content, author=current_user.username, created=datetime.now(), updated=datetime.now())
       db.session.add(post)
...
def edit(post_id):
	...
	if request.method == 'POST':
		try:		
			title = request.form['title']
			content = request.form['content']
			image = request.form['files']
			image2 = request.form['filez']
			post.title = title
